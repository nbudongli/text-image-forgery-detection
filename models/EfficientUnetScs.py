...
####### my model ###########
class EfficientUnetScs(nn.Module):
    def __init__(self, encoder, dropout_rate=0.5):
        super().__init__()
        self.decoder = Decoder(decoder_filters=encoder_params[encoder]["decoder_filters"],
                               filters=encoder_params[encoder]["filters"])
        self.avg_pool = AdaptiveAvgPool2d((1, 1))
        self.final = Conv2d(encoder_params[encoder]["decoder_filters"][0], out_channels=3, kernel_size=1, bias=True)
